...
export default function CreateWodModal({ initialWod = null, saving = false, onClose, onSave }) {
  ...
  return (
    <div className="fixed inset-0 z-[140] flex items-center justify-center p-2 sm:p-3">
      <button
        type="button"
      ...
      <form
        ...
        className="phoenix-card relative z-[141] flex max-h-[94dvh] w-full max-w-7xl flex-col overflow-y-auto xl:grid xl:h-[94vh] xl:grid-cols-[1.08fr_.92fr] xl:overflow-hidden"
      >
        <button
          type="button"
          onClick={onClose}
          className="absolute right-3 top-3 z-20 flex h-10 w-10 items-center justify-center rounded-xl border border-white/10 bg-black/60 text-xl text-white/70 transition hover:border-orange-400/40 hover:text-orange-300 sm:right-4 sm:top-4 xl:h-9 xl:w-9"
          aria-label="Cerrar"
        >
          ×
        </button>

        <section className="border-b border-orange-500/15 p-4 sm:p-5 xl:min-h-0 xl:overflow-y-auto xl:border-b-0 xl:border-r">
          <div className="mb-4 pr-10">
            <div className="inline-flex items-center gap-2 text-xs font-black uppercase tracking-[0.12em] text-orange-300">
          ...
          <div className="space-y-3">
            ...
            <Field label="Descripción del WOD" required>
              <textarea
                ...
                className="h-[145px] w-full resize-none rounded-2xl border border-white/10 bg-black/30 px-4 py-3 text-sm leading-6 text-white outline-none transition placeholder:text-white/30 focus:border-orange-500 focus:shadow-[0_0_0_4px_rgba(249,115,22,.15)] sm:h-[170px]"
              />
            </Field>
            ...
            <div className="rounded-2xl border border-orange-500/15 bg-orange-500/10 p-3">
              <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
                ...
                <button
                  ...
                  className="rounded-xl border border-orange-500/30 bg-black/35 px-4 py-2 text-sm font-black text-orange-300 transition hover:bg-orange-500/15 disabled:cursor-not-allowed disabled:opacity-50"
                >
              ...
              {aiError ? (
                <div className="mt-2 rounded-xl border border-red-500/25 bg-red-500/10 px-3 py-2 text-xs text-red-200">
                  {aiError}
                </div>
              ) : null}

              {estimate.source === "gemini" ? (
                <div className="mt-2 rounded-xl border border-green-500/20 bg-green-500/10 px-3 py-2 text-xs font-bold text-green-200">
          ...
          <div className="mt-4 grid grid-cols-1 gap-3 sm:flex sm:flex-row sm:justify-end">
            <button
              type="button"
              onClick={onClose}
              disabled={saving || aiLoading}
              className="phoenix-button-ghost w-full py-2 text-sm disabled:opacity-60 sm:w-auto"
            >
              Cancelar
            </button>
            ...
            <button
              type="submit"
              disabled={saving || aiLoading}
              className="phoenix-button-primary w-full py-2 text-sm disabled:opacity-60 sm:w-auto"
            >
        ...
        <section className="min-h-0 p-4 sm:p-5 xl:overflow-y-auto">
...
function Field({ label, required = false, children }) {
  return (
    <div>
      <label className="mb-1.5 block text-[11px] font-black uppercase tracking-[0.14em] text-white/55">
